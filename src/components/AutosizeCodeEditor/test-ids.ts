 ...
 *
 * Inlined from @volkovlabs/components (constants/tests.ts) to remove the
 * dependency. Each entry uses createSelector so that callers can use
 * `.apply()` to spread the appropriate data-testid / aria-label attributes
 * onto rendered nodes.
 */
